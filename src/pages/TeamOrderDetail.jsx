...
export default function TeamOrderDetail() {
  ...
  const handlePrintPDF = async () => {
    if (!pdfTemplate || !teamOrder || !workOrder) return;

    setSaving(true);
    try {
      console.log('Using PDF Template:', { id: pdfTemplate.id, name: pdfTemplate.template_name });
      ...
      if (response.data.success) {
        ...
        setError(`PDF generation failed: ${response.data.error}`);
      }
    } catch (err) {

...
import { StorageContractFullFlow } from '@/components/wizard/StorageContractFullFlow';
...
function WizardContent() {
  ...
  const names = {
    ...
    5: 'Intent',
    5.5: 'Project',
    6: wizardData.intent === 'storage_transport' ? 'Storage & Transport' : wizardData.intent === 'storage_contract' ? 'Storage Contract' : wizardData.intent === 'storage_contract_full' ? 'Angebot + Vertrag' : 'Details',
    7: 'Line Items',
    8: 'Technicians',
    9: 'Partner',
    10: 'Review'
  ...
  const isStepVisible = (step) => {
    ...
    // Step 6-7: Offer or Job details (NOT shown for storage flows — they have their own embedded flows)
    const isStorageFlow = wizardData.intent === 'storage_transport' || wizardData.intent === 'storage_contract' || wizardData.intent === 'storage_contract_full';
    if (step === 6) return !isStorageFlow;
    if (step === 7) return wizardData.intent && wizardData.intent.includes('offer') && !isStorageFlow;

    // Steps 8-9 not used for storage flows
    if (step === 8) return !isStorageFlow && (wizardData.intent === 'inspection' || wizardData.intent === 'workorder_for_existing_project' || (wizardData.intent && wizardData.intent.includes('job') && wizardData.workOrder?.createFirst !== false));
    if (step === 9) return !isStorageFlow && (wizardData.intent === 'inspection' || wizardData.intent === 'workorder_for_existing_project' || (wizardData.intent && wizardData.intent.includes('job')));
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          ...
          {wizardData.currentStep === 6 && wizardData.intent === 'storage_contract_full' && <StorageContractFullFlow />}
          {wizardData.currentStep === 6 && wizardData.intent !== 'storage_transport' && wizardData.intent !== 'storage_contract' && wizardData.intent !== 'storage_contract_full' && <Step6Details />}

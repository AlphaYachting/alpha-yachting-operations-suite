...
// ─── PREISDATEN ────────────────────────────────────────────────────────────────
// Quelle 1: Rumpf Bearbeitung Preisliste (DE) — Preise netto in EUR, exkl. 25% MwSt.
// Quelle 2: Cjenik vanjskog/unutarnjeg pranja te poliranja (HR) — übersetzt + normalisiert

const RUMPF_PREISE = [
  // lengthBand, SY_Vorb, SY_1st, SY_2nd, SY_Shaft, MY_Vorb, MY_1st, MY_2nd, MY_ShaftFlaps
...
const REINIGUNGS_PREISE = [
  // lengthBand, Außenwäsche_normal, Außenwäsche_Teak, Innen_normal, Innen_ausserordentlich, Politur, Politur_Metallic
...
// Alle Längenklassen (eindeutig)
const LENGTH_BANDS = RUMPF_PREISE.map(r => r[0]);

// ─── SERVICE-DEFINITIONEN ───────────────────────────────────────────────────────
const SERVICES = {
  // Reinigung / Politur — gilt für S/Y und M/Y
...
// Preisabruf pro Servicekey + Längenklasse
...
// ─── HAUPTKOMPONENTE ────────────────────────────────────────────────────────────
export default function PartnerKalkulator() {
  ...
  // Filter services by boatType + category
  ...
  // Reset selected services when boat type or category changes
  ...
  const fmt = (n) => n !== null && n !== undefined ? `EUR ${n.toLocaleString('de-DE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : '—';
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">Interne Preisabschätzung · Keine kaufmännische Verbindlichkeit</p>
          </div>
          <Badge variant="outline" className="ml-auto text-amber-700 border-amber-300 bg-amber-50">Intern</Badge>
        </div>
        ...
        <p className="text-xs text-center text-slate-400 pb-4">
          Nur zur internen Orientierung · Kein offizielles Angebot · Keine kaufmännische Verbindlichkeit
        </p>
      </div>
    </div>
  );

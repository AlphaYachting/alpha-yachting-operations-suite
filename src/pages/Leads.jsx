...
import { Phone, Mail, Anchor, MapPin, Plus, Edit, Trash2, CheckCircle2, Eye, Search, Clock, XCircle, Users as UsersIcon } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-600 mt-1">Manage customer inquiries and opportunities</p>
        </div>
        <Button onClick={() => {
          setEditingLead(null);
          setShowForm(true);
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { status: 'Pending', icon: Clock, color: 'text-amber-600' },
          { status: 'Contacted', icon: Phone, color: 'text-blue-600' },
          { status: 'Converted', icon: CheckCircle2, color: 'text-green-600' },
          { status: 'Lost', icon: XCircle, color: 'text-slate-600' }
            ...
            <Card key={status}>
              <CardContent className="pt-6">
                <div className="flex items-center justify-between">
                  ...
                  <Icon className={`h-8 w-8 ${color.replace('text-', 'text-').replace('600', '400')}`} />
                </div>
              </CardContent>
            </Card>
          );
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="text-center py-12">
                ...
                <p className="text-slate-600 mb-4">Get started by creating your first lead</p>
                <Button onClick={() => {
                  setEditingLead(null);
                  setShowForm(true);
                }} className="bg-blue-600 hover:bg-blue-700">
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-start gap-4 flex-1 min-w-0">
                    ...
                    <div className={`h-12 w-12 rounded-lg bg-gradient-to-br flex items-center justify-center flex-shrink-0 ${
                      lead.status === 'Pending' ? 'from-amber-50 to-orange-50' :
                      lead.status === 'Contacted' ? 'from-blue-50 to-cyan-50' :
                      lead.status === 'Converted' ? 'from-green-50 to-emerald-50' :
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 flex-wrap mb-2">
                        ...
                        {lead.inquiry_type && (
                          <Badge variant="outline" className={`border ${inquiryTypeColors[lead.inquiry_type]}`}>
                            {lead.inquiry_type}
                          </Badge>
                        )}
                      </div>

                      {/* Row 2: Contact Info, Boat, Location */}

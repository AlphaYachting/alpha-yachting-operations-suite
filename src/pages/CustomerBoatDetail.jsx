...
import { Ship, ArrowLeft, Clock, CheckCircle2, Pause, Calendar } from 'lucide-react';
...
export default function CustomerBoatDetail() {
  ...
  const loadData = async () => {
    try {
      const boatData = await base44.entities.Boat.filter({ id: boatId });
      if (boatData.length > 0) {
        setBoat(boatData[0]);
        
        // Load jobs for this boat
        ...
        {jobs.length === 0 ? (
          ...
          <div className="space-y-4">
            {jobs.map(job => {
              ...
              return (
                <Link key={job.id} to={createPageUrl('CustomerProjectDetail') + `?jobId=${job.id}`}>
